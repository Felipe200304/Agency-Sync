...
const roleInfo: Record<ApiAgencyLink['role'], { label: string; icon: typeof Home }> = {
  BASE:          { label: 'Agência base (residência)', icon: Home },
  MOTHER:        { label: 'Mother agency', icon: Star },
  INTERNATIONAL: { label: 'Internacional', icon: Globe },
  LOCAL:         { label: 'Representação local', icon: Building2 },
}
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {/* Agências & Comissões — vínculo multi-agência com papel e comissão configurável */}
            <div className="bg-card border border-border rounded-sm p-6">
              <p className="text-xs tracking-widest uppercase text-muted-foreground mb-5">Agências & Comissões</p>
              {links.length === 0 ? (
                <p className="text-sm text-muted-foreground">Nenhuma agência vinculada.</p>
              ) : (
                <div className="space-y-3">
                  {links.map(link => {
                    const info = roleInfo[link.role]
                    const Icon = info.icon
                    return (
                      <div key={link.id} className="flex items-center gap-3 border border-border rounded-sm p-3">
                        <div className="w-9 h-9 rounded-sm bg-primary/10 flex items-center justify-center flex-shrink-0">
                          <Icon className="w-4 h-4 text-primary" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-foreground truncate">{link.agencyName}</p>
                          <p className="text-xs text-muted-foreground">{info.label}</p>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <p className="text-sm font-medium text-primary">{Number(link.commissionPercent)}%</p>
